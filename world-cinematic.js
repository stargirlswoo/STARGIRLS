...
function marquee(){const rows=$$('.scene-marquee');rows.forEach(row=>{if(row.dataset.moving==='1'||reduced)return;row.dataset.moving='1';let x=0,last=performance.now();const step=now=>{const dt=Math.min(32,now-last);last=now;x+=dt*.025;const width=Math.max(0,row.scrollWidth-row.clientWidth);if(width){const loop=Math.max(1,row.scrollWidth/2);if(x>loop)x=0;row.scrollLeft=Math.min(x,width)}requestAnimationFrame(step)};requestAnimationFrame(step)})}
...
function clubHandoff(){let should=false;try{should=localStorage.getItem('stargirls-open-club-on-arrival-v1')==='1'}catch{}if(!should)return;let tries=0;const open=()=>{const b=$('[data-open-club][data-wired="1"]');if(b){try{localStorage.removeItem('stargirls-open-club-on-arrival-v1')}catch{}b.click();return}if(tries++<16)setTimeout(open,180)};open()}

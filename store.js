...
const brandMeta=p=>window.STARGIRLS_BRAND?.productMeta?.(p?.name||'')||{title:String(p?.name||'STARGIRLS PIECE').toUpperCase(),side:'joint',label:'moon + sun'};
...
function renderProducts(){
  ...
  if(!visible.length){productGrid.innerHTML='<div class="catalog-empty"><strong>nothing here yet.</strong><p>we are still making it.</p></div>';return;}
  productGrid.innerHTML=visible.map(p=>{
    ...
    const meta=brandMeta(p);
    const sub=String(p?.category||'').toLowerCase()==='perfume'?'juno':meta.label;
...
function renderCart(){
  ...
  if(!cart.length){cartItems.innerHTML='<div class="cart-empty"><strong>bag is empty.</strong><p>anything you add will show up here.</p></div>';cartSubtotal.textContent=money(0);checkoutButton.disabled=true;if(checkoutNote)checkoutNote.textContent='Add an item to continue to secure checkout.';return;}
  let total=0;
...
async function checkout(){
  if(!STORE_API_BASE||!cart.length)return;checkoutButton.disabled=true;checkoutButton.textContent='opening checkout…';

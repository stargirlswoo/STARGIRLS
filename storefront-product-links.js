/* STARGIRLS storefront: cards open exact current Printful products + compact conversion/Club layers. */
(function(){
  ...
  function ensureExperienceAssets(){
    ...
    if(!document.querySelector('script[data-store-club-handoff]')){
      const handoff=document.createElement('script');handoff.src='storefront-club-handoff.js?v=20260910a';handoff.defer=true;handoff.dataset.storeClubHandoff='1';document.body.appendChild(handoff);
    }
  }
  function productFor(id){try{return typeof products!=='undefined'?products.find(p=>p.id===id):null}catch{return null}}
  function destination(id){if(id==='juno-edp')return'fragrance.html';const p=productFor(id),dynamic=/^printful-(\d+)$/.exec(String(id||'')),pfid=Number(p?.printful_product_id||dynamic?.[1]||0),q=new URLSearchParams({v:'20260910s',id:String(id)});if(pfid>0)q.set('pf',String(pfid));return`product.html?${q.toString()}`}

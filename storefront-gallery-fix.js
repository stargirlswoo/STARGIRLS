/* STARGIRLS storefront gallery: real garment mockups only for Printful products. */
(function(){
  ...
  const isMockup=file=>!isArtwork(file)&&/mockup|front|back|side|sleeve|product/.test(meta(file));
  const score=file=>{const m=meta(file);let s=0;if(isArtwork(file))return-9999;if(/mockup/.test(m))s+=500;if(/front/.test(m))s+=220;if(/back/.test(m))s+=170;if(/side/.test(m))s+=140;if(/sleeve/.test(m))s+=100;if(/product/.test(m))s+=50;return s;};
  const imagesFromVariant=v=>unique((v?.files||[]).filter(f=>f?.preview_url&&isMockup(f)).slice().sort((a,b)=>score(b)-score(a)).map(f=>f.preview_url));
